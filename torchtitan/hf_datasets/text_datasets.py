...
# New private function to load WikiText-103-raw-v1
def _load_wikitext_dataset(dataset_path: str, split: str):
    """Load WikiText dataset with 'wikitext-103-raw-v1' configuration."""
    # We load the raw version for language modeling, similar to the C4 example.
...
def _process_wikitext_text(sample: dict[str, Any]) -> str:
    ...
    # WikiText uses the 'text' key, same as C4
    return sample["text"]
...
DATASETS = {
    ...
    # New entry for WikiText-103-raw-v1 (Training)
    "wikitext103": DatasetConfig(
        path="wikitext",  # The HuggingFace dataset name
        loader=partial(_load_wikitext_dataset, split="train"),
        sample_processor=_process_wikitext_text,
    ),
    # New entry for WikiText-103-raw-v1 (Validation)
    "wikitext103_validation": DatasetConfig(
        path="wikitext",  # The HuggingFace dataset name
