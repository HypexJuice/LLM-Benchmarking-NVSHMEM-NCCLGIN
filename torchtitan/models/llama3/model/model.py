...
from torchtitan.components.nvshmem_backend import get_nvshmem_all_to_all
...
class MoELayer(nn.Module):
    """
    Mixture of Experts layer with NVSHMEM-accelerated routing.
    """
    ...
    def __init__(
        ...
        # Create experts (simple FFN for each)
        self.experts = nn.ModuleList([
            FeedForward(dim=dim, hidden_dim=4 * dim, multiple_of=256, ffn_dim_multiplier=None)
            for _ in range(num_experts)
        ])
        
        # NVSHMEM all-to-all operator
        if use_nvshmem:
            self.all_to_all_op = get_nvshmem_all_to_all()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_nvshmem and self.all_to_all_op.ctx.initialized:
            output = self._route_with_nvshmem(x_flat, routing_weights, selected_experts)
        else:
            output = self._route_with_nccl(x_flat, routing_weights, selected_experts)
        
        return output.view(batch_size, seq_len, dim)
    
    def _route_with_nvshmem(
        self,
        x: torch.Tensor,
        routing_weights: torch.Tensor,
        selected_experts: torch.Tensor
    ) -> torch.Tensor:
        """Route tokens to experts using NVSHMEM."""
        num_tokens = x.shape[0]
        
        # Sort tokens by expert assignment for efficient batching
        # This is a simplified version - production code needs more sophisticated routing
        ...
        for expert_idx in range(self.num_experts):
            ...
            if mask.any():
                ...
                expert_outputs.append((mask, expert_output, routing_weights[mask]))
        
        # Combine expert outputs
        output = torch.zeros_like(x)
        for mask, expert_out, weights in expert_outputs:
            # Weight the expert output
            weighted_out = expert_out * weights.mean(dim=-1, keepdim=True)
            output[mask] += weighted_out
        
        return output
    
    def _route_with_nccl(
        self,
        x: torch.Tensor,
        routing_weights: torch.Tensor,
        selected_experts: torch.Tensor
    ) -> torch.Tensor:
        """Fallback routing with standard NCCL."""
        # Same logic as NVSHMEM version but using standard PyTorch ops
        return self._route_with_nvshmem(x, routing_weights, selected_experts)
...
class TransformerBlockWithMoE(nn.Module):
    ...
    def __init__(
        ...
        if use_moe:
            self.feed_forward = MoELayer(
                dim=model_args.dim,
                num_experts=model_args.num_experts,  # Add to ModelArgs
                num_experts_per_tok=model_args.num_experts_per_tok,  # Add to ModelArgs
                expert_group=expert_group,
                use_nvshmem=model_args.use_nvshmem,  # Add to ModelArgs
            )
        else:
            self.feed_forward = FeedForward(
                dim=model_args.dim,
    ...
    def forward(
        ...
        **kwargs
        ...
        h = x + self.attention(self.attention_norm(x), freqs_cis, **kwargs)
        out = h + self.feed_forward(self.ffn_norm(h))
        return out
